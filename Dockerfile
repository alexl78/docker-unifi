...
ENV PKGURL=https://dl.ubnt.com/unifi/5.6.20-11ecc42d69/unifi_sysvinit_all.deb
...
CMD ["/bin/bash", "-c", "chown -R unifi:unifi /var/lib/unifi /var/run/unifi /var/log/unifi && rm /var/run/unifi/unifi.pid && bash -x /etc/init.d/unifi start && exec /bin/bash"]
